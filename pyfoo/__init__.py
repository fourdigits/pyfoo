...
class Entry(UserDict):
    ...
    def comments(self):
        if not hasattr(self, '_comments'):
            self.setUrl()
            comments_json = self.form.api.make_call(
                            '%sforms/%s/comments.json?entryId=%s' % \
                            (self.base_url, self.form.Hash, self['EntryId']))
            self._comments = [Comment(self.form.api, comment) \
...
class Field(WufooObject):
    def __init__(self, api, json_object):
        ...
        if hasattr(self, 'Choices'):
            [setattr(choice, 'Score', None) for choice in self.Choices \
                                            if not hasattr(choice, 'Score')]
        else:
            self.Choices = []
...
class Form(WufooObject):
    def search_entries(self, parameters):
        ...
        for index in range(len(parameters)):
            ...
            output['Filter%s' % (index + 1)] = "%s__%s__%s" % \
    ...
    def get_entries(self, page_start=0, page_size=100, sort_field='DateCreated',
                        sort_direction='DESC', filter_string=None):
        url = str("%s?system=true&pageStart=%s&pageSize=%s&sort=%s"
                    "&sortDirection=%s" % (self.LinkEntries, page_start,
                           page_size, sort_field, sort_direction))
        if filter_string:
            url = "%s&%s" % (url, filter_string)
        entries_json = self.api.make_call(url)
        entries = [Entry(fields=entry, form=self) \
    ...
    def fields(self):
        if not hasattr(self, '_fields'):
            ...
            self._fields = [Field(self.api, field) \
    ...
    def comments(self):
        if not hasattr(self, '_comments'):
            comments_json = self.api.make_call(
                'https://%s.wufoo.com/api/v3/forms/%s/comments.json' % \
                    (self.api.account, self.Hash))
            self._comments = [Comment(self.api, comment) \
                                for comment in comments_json['Comments']]
        return self._comments
    ...
    def add_entry(self, entry):
        ...
        url = 'https://%s.wufoo.com/api/v3/forms/%s/entries.json' % \
                        (self.api.account, self.Hash)
    ...
    def add_web_hook(self, hook_url, handshake_key=None, send_metadata=True):
        post_params = {
            ...
            'metadata': str(send_metadata).lower()
        }
        url = 'https://%s.wufoo.com/api/v3/forms/%s/webhooks.json' % \
                    (self.api.account, self.Hash)
        response_json = self.api.make_call(url, post_params=post_params,
                                                method='PUT')
    ...
    def delete_web_hook(self, webhook_hash):
        ...
        url = 'https://%s.wufoo.com/api/v3/forms/%s/webhooks/%s.json' % \
                    (self.api.account, self.Hash, webhook_hash)
        response_json = self.api.make_call(url, post_params=post_params,
                                           method='DELETE')
...
class Report(WufooObject):
    ...
    def entries(self, page=None, count=None):
        if not hasattr(self, '_entries'):
            ...
            self._entries = [Entry(fields=entry, form=self) \
                             for entry in entries_json['Entries']]
        return self._entries

    def get_entries(self, page_start=0, page_size=100):
        url = "%s?system=true&pageStart=%s&pageSize=%s" % \
                (self.LinkEntries, page_start, page_size)
        entries_json = self.api.make_call(url)
        entries = [Entry(fields=entry, form=self) \
                   for entry in entries_json['Entries']]
        return entries
    ...
    def fields(self):
        if not hasattr(self, '_fields'):
            fields_json = self.api.make_call(self.LinkFields)
            self._fields = [Field(self.api, field) \
                            for field in fields_json['Fields'].values()]
        return self._fields
    ...
    def widgets(self):
        if not hasattr(self, '_widgets'):
            ...
            self._widgets = [Widget(self.api, widget) \
                             for widget in widgets_json['Widgets']]
        return self._widgets

    def get_link(self):
        return 'https://%s.wufoo.com/reports/%s/' % \
                    (self.api.account, self.Hash)
...
class PyfooAPI(object):
    ...
    def make_call(self, url, post_params=None, method=None):
        ...
        top_level_url = self.setUrl()
        password_mgr.add_password(None, top_level_url, self.api_key,
                                        "footastic")
...
#        test_script.close()
    ...
    def users(self):
        if not hasattr(self, '_users'):
            users_json = self.make_call('%susers.json' % self.setUrl())
            self._users = [User(self, user_dict) \
    ...
    def forms(self):
        if not hasattr(self, '_forms'):
            forms_json = self.make_call('%sforms.json' % self.setUrl())
    ...
    def reports(self):
        if not hasattr(self, '_reports'):
            reports_json = self.make_call('%sreports.json' % self.setUrl())
            self._reports = [Report(self, report_dict) \
                             for report_dict in reports_json['Reports']]
